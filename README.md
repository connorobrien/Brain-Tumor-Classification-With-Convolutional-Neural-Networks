...
The initial Convolution Layer is where we first apply our filters (kernels) to extract various features from the image volumes. In the case of our trial runs, we used 64 individual 3x3x3 kernels to scan the initial images, using a Rectified Linear Activation Unit (ReLU) activation function. This activation function is applied on every value of the output of the two-dimensional convolution output, or feature map. In general purpose, convolution of an image with different kernels can perform tasks like edge detection, blurring, and sharpening an image. ReLU is generally considered the default function for CNN-image classification problems due to its speed-to-train and strong performance; briefly defined, ReLU outputs the given input values where positive, and zero where negative, providing computational advantages like sparse matrices and trivial max functionality.

After the first round of convolution, we enter the Pooling Layer. The purpose of the pooling layer is dimensionality reduction of the feature map by deriving some summary statistic of nearby outputs—the most common of those statistics, and the method we used in our first pooling layer, is extracting the maximum output of a neighborhood of values, called max pooling. Using a three-dimensional set of feature maps from the convolutional layer, max pooling outputs one two-dimensional map per filter used, with reduced spatial dimensions. In our first round of max pooling, we used a pool-size of two, meaning that the maximum value from each non-overlapping 2x2 area of the set of feature maps were extracted.

On our first run of the data, we were only classifying two outcomes: the presence of Alzheimer’s disease, or not (Cognitively Normal). Thus, we were able to apply a binary cross-entropy loss function (log-loss). Essentially, this loss function, which takes the negative log of the predicted probability of an outcome, behaves in a such a way that as the predicted probability of the “1” (or “True” or “Positive”) class gets close to zero, the loss increases exponentially. Taking the mean of all of the losses of each prediction yields the binary cross-entropy, and this is precisely what we are trying to minimize.

The model was compiled with a binary cross entropy loss function and optimized using the Adam algorithm introduced by Kingma et al., 2014. Our Adam algorithm has an initial learning rate of 0.001 with a custom exponential decay schedule. The Adam algorithm is generally considered to be well-suited for deep learning, natural language processing, and, in our case, computer vision. It further has benefits for our use-case, as it is easy to implement and is computationally efficient. Adam combines features of momentum and adaptive learning rate algorithms to converge quickly without a loss of accuracy. 
...
Our final model achieved a 67% classification accuracy with a ROC area under curve of 69%. The model accuracy and loss for the training and validation sets are plotted below.
 ...
 <p align="center">
  <img src='https://raw.githubusercontent.com/connorobrienedu/Deep-Learning-for-Alzheimers-Disease-Detection/main/Images/model_statisticcs.png?token=ASLEFKPPVGPPHZPI67BHFTLAPDEIM' width="500">
</p>
